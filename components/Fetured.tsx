...
const Fetured = () => {
  ...
  return (
    <section className="max-w-screen-xl p-5 mx-auto pt-24 ">
      <Carousel className="border   h-fit shadow-2xl"
        opts={{
          align: "start",
          loop: true,
        }}
      >
        ...
        <CarouselContent>
          {posts.map((post) => (
            <CarouselItem key={post.id}>
              <div className="grid grid-cols-1 md:grid-cols-2 h-full   group ">
                <div className="flex flex-col gap-3 p-5 pt-24 ">

                  <div className="flex flex-col gap-2 mt-auto">
                    
                  <span>{post.category}</span>
                  <span>{post.userId}</span>
                <span className="text-sm text-muted-foreground">{new Date(post.publishedAt).toLocaleString()}</span>
                  </div>
                  <div className="flex flex-col gap-3" >

                  <Link href={`/${post.id}`} as={`/blogs/${post.id}`}>
                  <h1 className="text-xl md:text-2xl line-clamp-2 text-foreground group-hover:underline  group-hover:text-primary ">{post.title}</h1>
                  </Link>
                  <p className="line-clamp-3">{post.content}</p>

                  </div>
                  <div className="mt-auto border-t pt-5 group-hover:border-primary text-sm text-muted-foreground justify-between items-center flex gap-3 ">
                <p>0 Comments</p>
                ...
                <div className=" relative h-full aspect-square   overflow-hidden">
                 ...
                 fill
                    alt={post.title}
                    className=" group-hover:scale-110 object-cover object-center  transition-all duration-150"
                  />
                </div>
              </div>
            </CarouselItem>
        ...
        <div className="absolute top-14 left-16 gap-0   ">

           <CarouselPrevious className="border-[2px] rounded-none "  />
        <CarouselNext className="border-[2px] rounded-none "   />
        </div>
       <div>
        <h6 className=" font-bold text-xl border bg-background px-6 py-3 absolute -top-[4%] md:-top-[6%] lg:-top-[5%]  left-4">Fetured Posts</h6>
       </div>
      </Carousel>
    </section>
  );
